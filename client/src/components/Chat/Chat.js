...
function Chat({ socket, users, messages, currentUser, roomId }) {
	const [message, setMessage] = React.useState("");

	const onMessageSend = (e) => {
		e.preventDefault();
		socket.emit("CREATE_MESSAGE", { from: currentUser, text: message, roomId });
		setMessage("");
	};
	...
	return (
		<div className={styles.chat}>
			...
			<div className={styles.participants}>
				{users &&
					users.map((user) => (
						<div className={styles.participant} key={user.socketId}>
							{user.displayName}
						</div>
					))}
			</div>
			...
			<form className={styles.chatForm} onSubmit={onMessageSend}>
				<input
					type="text"
